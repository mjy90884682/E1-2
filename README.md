...
- `data/initial_state.json`: 최초 실행과 손상 복구에 사용하는 초기 상태
- `game.py`: 기능을 조율하고 상태 변경을 관리하는 `QuizGame`
- `storage.py`: `state.json` 읽기와 쓰기
- `console_ui.py`: 사용자 입력과 화면 출력
- `main.py`: 객체 조립, 메뉴 흐름과 안전한 종료

`percentage`는 저장하지 않고 `correct`와 `total`로부터 계산합니다. 저장된 상태가 없으면 `data/initial_state.json`으로 시작하며, 변경 시 기본 문제를 포함한 현재 전체 상태를 `state.json`에 저장합니다.
